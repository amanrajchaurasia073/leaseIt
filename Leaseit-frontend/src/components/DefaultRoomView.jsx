...
function DefaultRoomView({ data }) {
  ...
  // Filter only Room cards
  const roomData = data.filter((item) => item.type.toLowerCase() === "room");

  // Handle card click
  const handleCardClick = (id) => {
    navigate(`/room/${id}`); // Navigate to the details page with the card ID
  };
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 ml-36 mr-36">
      {roomData.map((item) => (
        <div
          key={item.id}
          className="bg-gray-300 rounded-lg shadow-md overflow-hidden cursor-pointer hover:shadow-lg transition-shadow"
